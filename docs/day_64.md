# Day 64: Design Twitter — Social Media Feed
> *Timeline at scale for hundreds of millions of users*
...
Today we explore **Design Twitter — Social Media Feed** — a fundamental concept you'll encounter when designing large-scale systems. Understanding this well is the difference between a system that breaks under load and one that scales gracefully.
...
**Design Twitter — Social Media Feed** is used in production systems at Google, Netflix, Uber, and Amazon. The concepts you learn today appear in system design interviews and every day in engineering work.
...
**Fan-out on Write**: This is the foundation. Without understanding this, the rest doesn't make sense. Take your time here.

**Fan-out on Read**: Once you have the foundation, this builds on top of it. You'll see this in almost every real-world system.

**Celebrity Problem**: This is where the real engineering happens. Companies spend months optimising this.
...
Think of **Design Twitter — Social Media Feed** like how a large airport operates:
...
class DesignTwitterSocialMediaFeedDemo {
  ...
  async _process(input) {
    ...
    // Core logic representing Fan-out on Write
...
async function runDemo() {
  const demo = new DesignTwitterSocialMediaFeedDemo();
...
class DesignTwitterSocialMediaFeedSystem:
    # Implementation of Design Twitter — Social Media Feed concepts
    # Demonstrates: Fan-out on Write, Fan-out on Read, Celebrity Problem

    def __init__(self, config: Dict = None):
        self.config = config or {'max_retries': 3, 'timeout': 5.0}
        self.stats = {'requests': 0, 'successes': 0, 'failures': 0, 'total_latency': 0.0}
        self._lock = threading.Lock()
        print(f"🚀 Design Twitter — Social Media Feed System initialized")
...
def run_demo():
    system = DesignTwitterSocialMediaFeedSystem()
    print('\n📊 Running 5 sample operations...')
...
1. **Research**: Find a real engineering blog post about Fan-out on Write (try engineering.atscale.com, netflixtechblog.com, or engineering.fb.com)
2. **Code Challenge**: Extend the example above to log every operation to a file with timestamps
3. **Design Exercise**: Draw a system diagram showing how Design Twitter — Social Media Feed fits into a ride-sharing app like Uber
4. **Trade-off Analysis**: What are 3 situations where you would NOT use this approach?
5. **Interview Practice**: Explain Design Twitter — Social Media Feed to someone with no tech background using only analogies
...
*← [Day 63](day_63.md) | [Index](README.md) | [Day 65](day_65.md) →*

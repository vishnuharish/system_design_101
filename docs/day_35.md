# Day 35: Common Attack Vectors
> *SQL Injection, XSS, CSRF, and how to stop them*
...
Today we explore **Common Attack Vectors** — a fundamental concept you'll encounter when designing large-scale systems. Understanding this well is the difference between a system that breaks under load and one that scales gracefully.
...
**Common Attack Vectors** is used in production systems at Google, Netflix, Uber, and Amazon. The concepts you learn today appear in system design interviews and every day in engineering work.
...
**OWASP Top 10**: This is the foundation. Without understanding this, the rest doesn't make sense. Take your time here.

**Parameterised Queries**: Once you have the foundation, this builds on top of it. You'll see this in almost every real-world system.

**Content Security Policy**: This is where the real engineering happens. Companies spend months optimising this.
...
Think of **Common Attack Vectors** like how a large airport operates:
...
// OWASP Top 10, Parameterised Queries, Content Security Policy

class CommonAttackVectorsDemo {
  constructor(config = {}) {
    this.config = { maxRetries: 3, timeout: 5000, ...config };
    this.stats = { requests: 0, successes: 0, failures: 0, latencyTotal: 0 };
    console.log(`🚀 Common Attack Vectors Demo initialized`);
  ...
  async _process(input) {
    ...
    // Core logic representing OWASP Top 10
...
async function runDemo() {
  const demo = new CommonAttackVectorsDemo();
...
class CommonAttackVectorsSystem:
    # Implementation of Common Attack Vectors concepts
    # Demonstrates: OWASP Top 10, Parameterised Queries, Content Security Policy

    def __init__(self, config: Dict = None):
        self.config = config or {'max_retries': 3, 'timeout': 5.0}
        self.stats = {'requests': 0, 'successes': 0, 'failures': 0, 'total_latency': 0.0}
        self._lock = threading.Lock()
        print(f"🚀 Common Attack Vectors System initialized")
...
def run_demo():
    system = CommonAttackVectorsSystem()
    print('\n📊 Running 5 sample operations...')
...
1. **Research**: Find a real engineering blog post about OWASP Top 10 (try engineering.atscale.com, netflixtechblog.com, or engineering.fb.com)
2. **Code Challenge**: Extend the example above to log every operation to a file with timestamps
3. **Design Exercise**: Draw a system diagram showing how Common Attack Vectors fits into a ride-sharing app like Uber
4. **Trade-off Analysis**: What are 3 situations where you would NOT use this approach?
5. **Interview Practice**: Explain Common Attack Vectors to someone with no tech background using only analogies
...
*← [Day 34](day_34.md) | [Index](README.md) | [Day 36](day_36.md) →*

# Day 89: Mock Interview 3 — Social Media Feed
> *Full practice interview with evaluation*

**Month 3: Design Practice › Week 12: Production and Interview Prep**  
...
Today we explore **Mock Interview 3 — Social Media Feed** — a fundamental concept you'll encounter when designing large-scale systems. Understanding this well is the difference between a system that breaks under load and one that scales gracefully.
...
**Mock Interview 3 — Social Media Feed** is used in production systems at Google, Netflix, Uber, and Amazon. The concepts you learn today appear in system design interviews and every day in engineering work.
...
**Fan-out Strategy Decision**: This is the foundation. Without understanding this, the rest doesn't make sense. Take your time here.

**Celebrity Problem Solution**: Once you have the foundation, this builds on top of it. You'll see this in almost every real-world system.

**Cache Invalidation**: This is where the real engineering happens. Companies spend months optimising this.
...
Think of **Mock Interview 3 — Social Media Feed** like how a large airport operates:
...
// Fan-out Strategy Decision, Celebrity Problem Solution, Cache Invalidation

class MockInterview3SocialMediaFeedDemo {
  constructor(config = {}) {
    this.config = { maxRetries: 3, timeout: 5000, ...config };
    this.stats = { requests: 0, successes: 0, failures: 0, latencyTotal: 0 };
    console.log(`🚀 Mock Interview 3 — Social Media Feed Demo initialized`);
  ...
  async _process(input) {
    ...
    // Core logic representing Fan-out Strategy Decision
...
async function runDemo() {
  const demo = new MockInterview3SocialMediaFeedDemo();
...
class MockInterview3SocialMediaFeedSystem:
    # Implementation of Mock Interview 3 — Social Media Feed concepts
    # Demonstrates: Fan-out Strategy Decision, Celebrity Problem Solution, Cache Invalidation

    def __init__(self, config: Dict = None):
        self.config = config or {'max_retries': 3, 'timeout': 5.0}
        self.stats = {'requests': 0, 'successes': 0, 'failures': 0, 'total_latency': 0.0}
        self._lock = threading.Lock()
        print(f"🚀 Mock Interview 3 — Social Media Feed System initialized")
...
def run_demo():
    system = MockInterview3SocialMediaFeedSystem()
    print('\n📊 Running 5 sample operations...')
...
1. **Research**: Find a real engineering blog post about Fan-out Strategy Decision (try engineering.atscale.com, netflixtechblog.com, or engineering.fb.com)
2. **Code Challenge**: Extend the example above to log every operation to a file with timestamps
3. **Design Exercise**: Draw a system diagram showing how Mock Interview 3 — Social Media Feed fits into a ride-sharing app like Uber
4. **Trade-off Analysis**: What are 3 situations where you would NOT use this approach?
5. **Interview Practice**: Explain Mock Interview 3 — Social Media Feed to someone with no tech background using only analogies
...
*← [Day 88](day_88.md) | [Index](README.md) | [Day 90](day_90.md) →*

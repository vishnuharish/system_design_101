# Day 85: Cost Optimisation in System Design
> *Building systems that are efficient and cheap to run*
...
Today we explore **Cost Optimisation in System Design** — a fundamental concept you'll encounter when designing large-scale systems. Understanding this well is the difference between a system that breaks under load and one that scales gracefully.
...
**Cost Optimisation in System Design** is used in production systems at Google, Netflix, Uber, and Amazon. The concepts you learn today appear in system design interviews and every day in engineering work.
...
**Reserved vs Spot Instances**: This is the foundation. Without understanding this, the rest doesn't make sense. Take your time here.

**Right-Sizing**: Once you have the foundation, this builds on top of it. You'll see this in almost every real-world system.

**Caching for Cost**: This is where the real engineering happens. Companies spend months optimising this.
...
Think of **Cost Optimisation in System Design** like how a large airport operates:
...
// Reserved vs Spot Instances, Right-Sizing, Caching for Cost

class CostOptimisationinSystemDesignDemo {
  constructor(config = {}) {
    this.config = { maxRetries: 3, timeout: 5000, ...config };
    this.stats = { requests: 0, successes: 0, failures: 0, latencyTotal: 0 };
    console.log(`🚀 Cost Optimisation in System Design Demo initialized`);
  ...
  async _process(input) {
    ...
    // Core logic representing Reserved vs Spot Instances
...
async function runDemo() {
  const demo = new CostOptimisationinSystemDesignDemo();
...
class CostOptimisationinSystemDesignSystem:
    # Implementation of Cost Optimisation in System Design concepts
    # Demonstrates: Reserved vs Spot Instances, Right-Sizing, Caching for Cost

    def __init__(self, config: Dict = None):
        self.config = config or {'max_retries': 3, 'timeout': 5.0}
        self.stats = {'requests': 0, 'successes': 0, 'failures': 0, 'total_latency': 0.0}
        self._lock = threading.Lock()
        print(f"🚀 Cost Optimisation in System Design System initialized")
...
def run_demo():
    system = CostOptimisationinSystemDesignSystem()
    print('\n📊 Running 5 sample operations...')
...
1. **Research**: Find a real engineering blog post about Reserved vs Spot Instances (try engineering.atscale.com, netflixtechblog.com, or engineering.fb.com)
2. **Code Challenge**: Extend the example above to log every operation to a file with timestamps
3. **Design Exercise**: Draw a system diagram showing how Cost Optimisation in System Design fits into a ride-sharing app like Uber
4. **Trade-off Analysis**: What are 3 situations where you would NOT use this approach?
5. **Interview Practice**: Explain Cost Optimisation in System Design to someone with no tech background using only analogies
...
*← [Day 84](day_84.md) | [Index](README.md) | [Day 86](day_86.md) →*

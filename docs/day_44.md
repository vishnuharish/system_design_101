# Day 44: Week 6 Project — Microservice Chat App
> *Split a monolith into three independent services*

**Month 2: Security and Architecture › Week 6: Microservices Architecture**  
...
Today we explore **Week 6 Project — Microservice Chat App** — a fundamental concept you'll encounter when designing large-scale systems. Understanding this well is the difference between a system that breaks under load and one that scales gracefully.
...
**Week 6 Project — Microservice Chat App** is used in production systems at Google, Netflix, Uber, and Amazon. The concepts you learn today appear in system design interviews and every day in engineering work.
...
**User Service**: This is the foundation. Without understanding this, the rest doesn't make sense. Take your time here.

**Message Service**: Once you have the foundation, this builds on top of it. You'll see this in almost every real-world system.

**Notification Service**: This is where the real engineering happens. Companies spend months optimising this.
...
Think of **Week 6 Project — Microservice Chat App** like how a large airport operates:
...
class Week6ProjectMicroserviceChatAppDemo {
  constructor(config = {}) {
    this.config = { maxRetries: 3, timeout: 5000, ...config };
    this.stats = { requests: 0, successes: 0, failures: 0, latencyTotal: 0 };
    console.log(`🚀 Week 6 Project — Microservice Chat App Demo initialized`);
  ...
  async _process(input) {
    ...
    // Core logic representing User Service
...
async function runDemo() {
  const demo = new Week6ProjectMicroserviceChatAppDemo();
...
class Week6ProjectMicroserviceChatAppSystem:
    # Implementation of Week 6 Project — Microservice Chat App concepts
    # Demonstrates: User Service, Message Service, Notification Service

    def __init__(self, config: Dict = None):
        self.config = config or {'max_retries': 3, 'timeout': 5.0}
        self.stats = {'requests': 0, 'successes': 0, 'failures': 0, 'total_latency': 0.0}
        self._lock = threading.Lock()
        print(f"🚀 Week 6 Project — Microservice Chat App System initialized")
...
def run_demo():
    system = Week6ProjectMicroserviceChatAppSystem()
    print('\n📊 Running 5 sample operations...')
...
1. **Research**: Find a real engineering blog post about User Service (try engineering.atscale.com, netflixtechblog.com, or engineering.fb.com)
2. **Code Challenge**: Extend the example above to log every operation to a file with timestamps
3. **Design Exercise**: Draw a system diagram showing how Week 6 Project — Microservice Chat App fits into a ride-sharing app like Uber
4. **Trade-off Analysis**: What are 3 situations where you would NOT use this approach?
5. **Interview Practice**: Explain Week 6 Project — Microservice Chat App to someone with no tech background using only analogies
...
*← [Day 43](day_43.md) | [Index](README.md) | [Day 45](day_45.md) →*

# Day 67: Week 9 Patterns Review
> *Common patterns across all classic system designs*
...
Today we explore **Week 9 Patterns Review** — a fundamental concept you'll encounter when designing large-scale systems. Understanding this well is the difference between a system that breaks under load and one that scales gracefully.

### Core Topics

- **Write-Heavy vs Read-Heavy Patterns**
- **SQL vs NoSQL Selection Guide**
- **CDN Pattern**
- **Queue Pattern**
- **Cache Pattern**
...
**Week 9 Patterns Review** is used in production systems at Google, Netflix, Uber, and Amazon. The concepts you learn today appear in system design interviews and every day in engineering work.
...
**Write-Heavy vs Read-Heavy Patterns**: This is the foundation. Without understanding this, the rest doesn't make sense. Take your time here.

**SQL vs NoSQL Selection Guide**: Once you have the foundation, this builds on top of it. You'll see this in almost every real-world system.

**CDN Pattern**: This is where the real engineering happens. Companies spend months optimising this.
...
Think of **Week 9 Patterns Review** like how a large airport operates:
...
// Write-Heavy vs Read-Heavy Patterns, SQL vs NoSQL Selection Guide, CDN Pattern

class Week9PatternsReviewDemo {
  constructor(config = {}) {
    this.config = { maxRetries: 3, timeout: 5000, ...config };
    this.stats = { requests: 0, successes: 0, failures: 0, latencyTotal: 0 };
    console.log(`🚀 Week 9 Patterns Review Demo initialized`);
  ...
  async _process(input) {
    ...
    // Core logic representing Write-Heavy vs Read-Heavy Patterns
...
async function runDemo() {
  const demo = new Week9PatternsReviewDemo();
...
class Week9PatternsReviewSystem:
    # Implementation of Week 9 Patterns Review concepts
    # Demonstrates: Write-Heavy vs Read-Heavy Patterns, SQL vs NoSQL Selection Guide, CDN Pattern

    def __init__(self, config: Dict = None):
        self.config = config or {'max_retries': 3, 'timeout': 5.0}
        self.stats = {'requests': 0, 'successes': 0, 'failures': 0, 'total_latency': 0.0}
        self._lock = threading.Lock()
        print(f"🚀 Week 9 Patterns Review System initialized")
...
def run_demo():
    system = Week9PatternsReviewSystem()
    print('\n📊 Running 5 sample operations...')
...
1. **Research**: Find a real engineering blog post about Write-Heavy vs Read-Heavy Patterns (try engineering.atscale.com, netflixtechblog.com, or engineering.fb.com)
2. **Code Challenge**: Extend the example above to log every operation to a file with timestamps
3. **Design Exercise**: Draw a system diagram showing how Week 9 Patterns Review fits into a ride-sharing app like Uber
4. **Trade-off Analysis**: What are 3 situations where you would NOT use this approach?
5. **Interview Practice**: Explain Week 9 Patterns Review to someone with no tech background using only analogies
...
*← [Day 66](day_66.md) | [Index](README.md) | [Day 68](day_68.md) →*
